$(document).ready(function() {
    ...
    // 3. Animación en el carrusel al mostrar slide (usando 'slid.bs.carousel')
    $('#carouselExampleInterval').on('slid.bs.carousel', function (e) {
        var $active = $(this).find('.carousel-item.active');
        $active.find('.carousel-hover-tab p, .carousel-hover-tab a').css({opacity:0, position:'relative', top:'30px'});
        $active.find('.carousel-hover-tab p, .carousel-hover-tab a').each(function(i) {
